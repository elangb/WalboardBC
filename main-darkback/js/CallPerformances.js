        ...
        labels: {
          style: {
            colors: [
              ...
              "#FFFFFF",
              "#FFFFFF",
            ], // Set white color for all x-axis labels (days)
          },
        },
      },
      colors: ["#009947", "#00a0e8"],
      title: {
        text: "",
      },
      dataLabels: {
...
function barchart(Recieve, answer, abandon) {
  ...
  var datajson = [
    { name: "Recieve", data: [Recieve] },
    { name: "answer", data: [answer] },
    { name: "abandon", data: [abandon] },
  ];
...
function getData() {
  $.getJSON("PHP/DataDaily.php", function (data) {
    ...
    if (
      Array.isArray(splitSkills) &&
      Array.isArray(informasi) &&
      Array.isArray(pengaduan)
    ) {
      if (
        splitSkills.length === informasi.length &&
        splitSkills.length === pengaduan.length
      ) {
        ...
        combinedData.forEach((item) => {
          ...
          $("#received").html(
            Number(abandoned) + Number($("#answered").html())
          );
        ...
        barchart(
          Number($("#received").html()),
          Number($("#answered").html()),
          Number($("#abandoned").html())
        );
      }
    }
...
//     ".date-time-text"
//   ).textContent = `${day} | ${date} ${month} ${year} | ${hours}:${minutes}`;
// }
