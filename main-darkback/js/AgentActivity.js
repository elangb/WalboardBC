...
function ListAgent() {
  ...
  var jqxhr = $.getJSON("PHP/Agent_Activity.php", function (data) {
    console.log(data);

    const agents = data.Head;
    ...
    for (const key in agents) {
      if (key !== "Split Skill") {
        ...
        if (items[3] == "Avaliable")
          ...
          table +=
            '<td><span class="badge badge-danger">' + items[3] + "</span></td>";
        else if (items[3] == "AUX")
          table +=
            '<td><span class="badge badge-danger">' + items[3] + "</span></td>";
        else
          table +=
            '<td><span class="badge badge-light">' + items[3] + "</span></td>";

        table +=
          '<td><span class="badge badge-green">' + items[6] + "</span></td>";
      }
    }
